...
/obj/item/construction/rcd/proc/check_above(mob/user) //players should be able to deconstruct immediately above. (this is here if you want to get this to work with the arcd)
	var/turf/T = get_turf(user)
	T = SSmapping.get_turf_above(T)
	if(is_station_level(T.z) && target_check(T, user))
		return TRUE
	else
		return FALSE
...
/obj/item/construction/rcd/CtrlClick(mob/user, proximity) //Breaks ceilings
    if(isturf(loc)) //anti-pull
        return ..()
    var/turf/T = SSmapping.get_turf_above(get_turf(src))
    if(!T || !prox_check(proximity) || !mode==RCD_DECONSTRUCT)
        return ..()
    to_chat(user, "<span class='warning'>You begin to deconstruct the ceiling...</span>")
    rcd_create(T, user)
